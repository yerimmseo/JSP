...
<body>
	...
	<p>JSP객체의 수명은 설정된 Scope에 따라 달라진다.</p>
	...
	<dl>
		...
		<dt>session</dt>
		<dd>사용자의 세션이 만료될 때까지 수명이 유지된다. 세션은 브라우저 별로 존재한다.<br>
			IP주소가 같더라도 다른 브라우저를 사용하면 서버로부터 새로운 세션을 받아갈 수 있다.</dd>
		<dt>application</dt>
		<dd>어플리케이션 스코프에 저장하는 어트리뷰트는 서버가 꺼질 때까지 유지된다.</dd>
	</dl>
	...
	<ol>
		...
		<li>해당 웹서버에 다시 요청을 보낼 때 브라우저에 저장된 세션ID가 함께 전송된다.
			웹 서버는 해당 세션ID를 이용해 사용자를 식별한다.</li>
